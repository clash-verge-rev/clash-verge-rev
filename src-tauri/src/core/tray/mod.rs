...
fn create_tray_menu(
    ...
    let profile_items: Vec<CheckMenuItem<Wry>> = profile_names
        .iter()
        .map(|item| {
        let is_current_groups = Config::profiles().data().is_current_groups(item);
    ...
    let profile_menu_items: Vec<&dyn IsMenuItem<Wry>> = profile_items
        .iter()
...
fn on_menu_event(_: &AppHandle, event: MenuEvent) {
    match event.id.as_ref() {
        ...
        id if id.starts_with("profiles_") => {
            let group_name = &id["profiles_".len()..];
            feat::toggle_proxy_group(group_name.into());
        }
        _ => {}
    }
}

...
impl Config {
    ...
    pub async fn generate_file(typ: ConfigType) -> Result<PathBuf> {
        ...
        // 配置文件检测阶段不需要关心 IPC 路径，避免权限问题/不存在 导致无法验证
        if typ == ConfigType::Check {
            #[cfg(unix)]
        ...
        match typ {
            ConfigType::Run => {
                help::save_yaml(
                    &path,
                    &config,
                    Some("# Generated by Clash Verge for runtime"),
                )
                .await?
            }
            ConfigType::Check => {
                help::save_yaml(
                    &path,
                    &config,
                    Some("# Generated by Clash Verge for config validation"),
                )
                .await?
            }
        }

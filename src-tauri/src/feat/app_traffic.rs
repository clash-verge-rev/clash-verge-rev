    ...
    AsyncHandler::spawn(|| async {
        ...
        loop {
            ...
            let connections = match mihomo.get_connections().await {
                Ok(c) => c,
                Err(e) => {
                    logging!(trace, Type::Core, "App traffic daemon: failed to get connections: {}", e);
                    continue;
                }
            };
            ...
            if let Some(conns) = connections.connections {
                for conn in conns {
                    ...
                    let is_direct = conn.chains.iter().any(|c| c.eq_ignore_ascii_case("direct")) || conn.rule.eq_ignore_ascii_case("direct");
                    let is_reject = conn.chains.iter().any(|c| c.eq_ignore_ascii_case("reject")) || conn.rule.eq_ignore_ascii_case("reject");
                ...
                for (key, total_up) in &current_upload {
                    ...
                    let delta_up = if total_up > prev_up { total_up - prev_up } else { *total_up };
                    
                    let total_down = current_download.get(key).unwrap_or(&0);
                    let prev_down = last_download.get(key).unwrap_or(&0);
                    let delta_down = if total_down > prev_down { total_down - prev_down } else { *total_down };

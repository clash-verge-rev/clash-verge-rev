...
pub async fn create_backup_and_upload_webdav() -> Result<()> {
    ...
    if let Err(err) = temp_file_path.remove_if_exists() {
...
pub async fn restore_webdav_backup(filename: String) -> Result<()> {
    ...
    // 最后删除临时文件
    backup_storage_path.remove_if_exists()?;
    Ok(())
}
...
pub fn create_local_backup() -> Result<()> {
    ...
    if let Err(err) = move_file(temp_file_path.clone(), target_path.clone()) {
        ...
        // 清理临时文件
        if let Err(clean_err) = temp_file_path.remove_if_exists() {
            logging!(
                warn,
                Type::Backup,

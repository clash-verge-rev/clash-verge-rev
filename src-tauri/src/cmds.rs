...
/// 根据group name修改profiles
#[tauri::command]
pub async fn patch_profiles_config_by_group(group_name: String) -> CmdResult {
    let group_id = Config::profiles().data().get_groups_uid(&group_name);
    let profiles = IProfiles{current: group_id, items: None};

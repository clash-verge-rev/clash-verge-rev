...
/// - `data`: Returns an immutable reference to the committed data.
/// - `draft`: Creates or retrieves a mutable reference to the draft data, cloning the committed data if no draft exists.
/// - `latest`: Returns an immutable reference to the draft data if it exists, otherwise to the committed data.
/// - `latest`: Returns an mutable reference to the draft data if it exists, otherwise to the committed data.
...
impl<T: Clone + ToOwned> Draft<T> {
    /// 可写正式数据
    pub fn data_mut(&self) -> MappedRwLockWriteGuard<'_, T> {
    ...
    pub fn latest_mut(&self) -> MappedRwLockWriteGuard<'_, T> {
        RwLockWriteGuard::map(self.inner.write(), |inner| {
            inner.1.as_mut().unwrap_or(&mut inner.0)
        })
    }
    ...
    where
        T: Send + Sync + 'static,
        F: FnOnce(T) -> Fut + Send,
        Fut: std::future::Future<Output = Result<(T, R), E>> + Send,
        E: From<anyhow::Error>,
    {
        // 克隆正式数据
        let local = {

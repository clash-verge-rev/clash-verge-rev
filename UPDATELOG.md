...
- **流量监控系统重构**：实现全新的增强流量监控系统，支持数据压缩、采样和智能缓存
- **数据验证机制**：引入类型安全的数据验证器，确保 API 响应数据的一致性和可靠性

### ✨ 新增功能

- 增加 `Verge Version` 复制按钮
- 新增增强型流量监控 Hook (`useTrafficMonitorEnhanced`)，支持高级数据管理
- 实现 `TrafficDataSampler` 类，支持原始和压缩流量数据处理
- 添加引用计数管理器，根据组件使用情况智能管理数据收集
- 新增流量监控诊断工具，提供性能监控和错误追踪
- 支持流量数据的智能压缩和时间范围查询
- 增加错误边界组件，提升流量监控的稳定性
- 新增多版本画布流量图表组件，提供更丰富的可视化选项
...
- 更新 kode-bridge 依赖至 v0.1.5，提升 IPC 功能
- 移除过时的 MihomoManager，统一使用 IpcManager
- 重构流量和内存监控服务，移除 TrafficService 和 TrafficManager
- 添加外部控制器配置和 UI 支持

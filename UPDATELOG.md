...
> [!NOTE]
> 在原 clash-verge-rev 相同版本（没有`-patch`后缀）的基础上，添加的一些功能和改进
...
- 左侧菜单栏选中时图标高亮
- 支持UI界面是否保持活跃状态设置
- 使用 monaco 编辑器编辑主题设置的 CSS 样式注入
- 代理组界面添加右侧边栏展开显示代理组列表，实现点击跳转
- 配置文件新建/编辑的文件类型使用更醒目的按钮组
- 为重新激活配置文件，启用/关闭/删除增强文件(merge/script)操作添加动画

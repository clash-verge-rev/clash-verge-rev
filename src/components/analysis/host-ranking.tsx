...
interface HostRankingProps {
  hostStats: Map<string, { upload: number; download: number; count: number }>;
}

export const HostRanking = ({ hostStats }: HostRankingProps) => {
  const { t } = useTranslation();

  // 按总流量排序
  const topHosts = useMemo(() => {
    const hosts = Array.from(hostStats.entries()).map(([host, stats]) => ({
      host,
    ...
    return hosts.sort((a, b) => b.total - a.total).slice(0, 10);
  }, [hostStats]);

  const maxTraffic = topHosts[0]?.total || 1;
      ...
      {topHosts.map((item, index) => (
        <Box
          ...
          sx={{
            mb: 2,
            pb: 2,
            borderBottom: index < topHosts.length - 1 ? "1px solid" : "none",
            borderColor: "divider",
          }}
        >
          <Box

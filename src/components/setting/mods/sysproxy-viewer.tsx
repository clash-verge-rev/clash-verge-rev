...
export const SysproxyViewer = forwardRef<DialogRef>((_props, ref) => {
  ...
  const onSave = useLockFn(async () => {
    ...
    if (
      !IPV4_RE.test(value.proxy_host) &&
      !IPV6_RE.test(value.proxy_host) &&
      !HOSTNAME_RE.test(value.proxy_host)
    ) {
        ...
        <ListItem sx={{ padding: '5px 2px' }}>
          ...
          <TextField
            ...
            onChange={(e) => {
              setValue((v) => ({
                ...v,
                duration: +e.target.value.replace(NON_DIGIT_RE, ''),
              }))
            }}
          />
        </ListItem>

...
import { useClashConfigData, useRefreshers } from '@/providers/app-data-context'
import {
...
import {
  useProxyChainConfigData,
  useProxyChainExitNode,
  useProxyChainModeEnabled,
  useSetProxyChainConfigData,
  useSetProxyChainModeEnabled,
...
const ProxyPage = () => {
  ...
  const isChainMode = useProxyChainModeEnabled()
  const setIsChainMode = useSetProxyChainModeEnabled()
  const chainConfigData = useProxyChainConfigData()
  const updateChainConfigData = useSetProxyChainConfigData()
  const chainExitNode = useProxyChainExitNode()
  const { clashConfig } = useClashConfigData()
  const { refreshClashConfig } = useRefreshers()
  ...
  useEffect(() => {
    ...
    const fetchChainConfig = async () => {
      try {
        if (!chainExitNode) {
          console.error('No proxy chain exit node found')
          if (!cancelled) {
        ...
        const configData = await getRuntimeProxyChainConfig(chainExitNode)
        if (!cancelled) {
  ...
  }, [chainExitNode, isChainMode, updateChainConfigData])
